...
filename_rslt = os.path.join(desktop_path, 'train_' + sensorname + 'output5.txt')


# send data to cuda
y_train_torch = torch.from_numpy(y_train).to(torch.float32).to('cuda')
X_train_torch = torch.from_numpy(X_train).to(torch.float32).to('cuda')
y_test_torch = torch.from_numpy(y_test).to(torch.float32).to('cuda')
X_test_torch = torch.from_numpy(X_test).to(torch.float32).to('cuda')
...
model = sensorNet().to('cuda')
print(model)

def new_loss(input, target):
    # bb = (input[:, :3]-target[:, :3])
    # aa = (((input[:, :3]-target[:, :3])/target[:, :3])**2).mean() + (((input[:, 3:5]-target[:, 3:5]))**2).mean() 
    return (((input[:, :3]-target[:, :3])/(np.maximum(target[:, :3],[0.1,0.1,0.1])))**2).mean() + (((input[:, 3:5]-target[:, 3:5]))**2).mean() 

def old_loss(input, target):
...
with open(filename_rslt,'w') as g:
    ...
    for epoch in range(20):
        ...
        for X_batch, y_batch in loader:
            ...
            loss.backward()
            optimizer.step()
        if math.isnan(np.mean(batch_losses)):
            print("is nan")
...
filename = os.path.join(desktop_path, sensorname + 'FULL5.txt')
